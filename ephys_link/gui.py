...
class GUI:
    ...
    def __init__(self, root: Tk, launch_func: callable,
                 manipulator_stop_func: callable, poll_serial_func: callable,
    ...
    def build_gui(self):
        ...
        ttk.Entry(server_serving_settings, textvariable=server_port, width=5,
                  justify=CENTER).grid(column=1, row=1, sticky="we")

        # ---

        # Platform type
        platform_type_settings = ttk.LabelFrame(mainframe, text="Platform Type",
                                                padding=3)
        platform_type_settings.grid(column=0, row=1, sticky="news")

        ttk.Radiobutton(platform_type_settings, text='Sensapex uMp',
                        variable=platform_type, value='sensapex').grid(column=0, row=0,
                                                                       sticky="we")
        ttk.Radiobutton(platform_type_settings, text="New Scale",
                        variable=platform_type, value='new_scale').grid(column=0,
                                                                        row=1,
                                                                        sticky="we")
        ...
        # New Scale Settings
        new_scale_settings = ttk.LabelFrame(mainframe, text="New Scale settings",
                                            padding=3)
        new_scale_settings.grid(column=0, row=2, sticky="news")

        # Port
        ttk.Label(new_scale_settings, text="HTTP Server Port:", anchor=E,
                  justify=RIGHT).grid(column=0, row=1, sticky="we")
        ttk.Entry(new_scale_settings, textvariable=new_scale_port, width=5,
                  justify=CENTER).grid(column=1, row=1, sticky="we")

        # ---

        # Emergency Stop serial port
        e_stop_settings = ttk.LabelFrame(mainframe, text="Emergency Stop Settings",
                                         padding=3)
        e_stop_settings.grid(column=0, row=3, sticky="news")

        # Serial Port
        ttk.Label(e_stop_settings, text="Serial Port:", anchor=E,
                  justify=RIGHT).grid(column=0, row=1, sticky="we")
        ttk.Entry(e_stop_settings, textvariable=e_stop_serial_port,
                  justify=CENTER).grid(column=1, row=1, sticky="we")

        # Server start/stop button
        ttk.Button(mainframe, textvariable=server_launch_button_text,
    ...
    def start_stop_server(self, start: bool) -> None:
        ...
        if start:
            # Launch serial
            Thread(target=self._poll_serial_func,
                   args=(self._kill_serial_event, e_stop_serial_port.get(),),
                   daemon=True).start()
            # Launch server
            Thread(target=self._launch_func, args=(
                platform_type.get(), server_port.get(),
                new_scale_port.get()), daemon=True).start()
